...
// @brief 利用可能なページを見つける．
vector<SimpleAlloc::Page>::iterator
SimpleAlloc::find_page(SizeType alloc_size)
{
  for ( vector<Page>::iterator p = mAvailList.begin(); p != mAvailList.end(); ++ p ) {
    auto& page{*p};
